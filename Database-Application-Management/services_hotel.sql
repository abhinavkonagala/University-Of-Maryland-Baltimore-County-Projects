...
/*creating a procedure service_hotel to display all the information on all the services that each hotel has*/ 
...
if p_service=1 then
   ...
   for x in c1 loop
         ...
         END IF; 
dbms_output.put_line('Hotel ID: ' || x.hotel_id || ' Hotel Name :' || x.hotel_name || 'Reservation ID:  '  || x.reservation_id  ||'  | Number of Meals : ' || 
...
/*asks user input. If 2, it will display the number of laundry service in all the hotels*/
   ...
   for y in c2 loop
         if c2%NOTFOUND THEN
             dbms_output.put_line('There are no services for this reservation!');
         END IF; 
dbms_output.put_line('Hotel ID: ' || y.hotel_id || ' Hotel Name :' || y.hotel_name || 'Reservation ID :  '  || y.reservation_id  ||'  | Number of Laundry : ' || 
...
/*asks user input. If 3, it will display the number of movies service in all the hotels*/
   ...
   for z in c3 loop
         if c3%NOTFOUND THEN
             dbms_output.put_line('There are no services for this reservation!');
         END IF; 
dbms_output.put_line('Hotel ID: ' || z.hotel_id || ' Hotel Name :' || z.hotel_name || 'Reservation ID :  '  || z.reservation_id  ||'  | Number of Movies :   ' 
...
exec services_hotel(&Service_Name)
